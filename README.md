...
- Installed-model switching now releases resident LLM and diffusion runners before the final resource snapshot and assessment; every confirmation/retry/alternative is bound to its exact load generation, model, and mode, while CPU alternatives require explicit non-CPU candidate provenance and separate assessment
