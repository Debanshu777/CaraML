...
- Model downloads now use a persistent resumable queue with exact-artifact verification; Android uses UIDT/foreground notifications, iOS restores a per-task exact response-byte bound and cancels oversized known or chunked responses, and Desktop resumes on relaunch
