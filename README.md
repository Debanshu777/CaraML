...
- Case-distinct owner repairs never coalesce, cancellation remains cancellation during queued manifest checks and publication/root-lock waits, and every Model Hub download now requires the durable coordinator with no direct Ready publication fallback
...
- Installed-model switching now releases resident LLM and diffusion runners before the final resource snapshot and assessment; accelerator memory/no-plan failures may offer only a separately assessed exact CPU request behind explicit confirmation, while stale or incompatible evidence remains blocked
