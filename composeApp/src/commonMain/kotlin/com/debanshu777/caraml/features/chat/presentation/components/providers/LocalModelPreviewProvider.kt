...
class LocalModelPreviewProvider : PreviewParameterProvider<LocalModelEntity> {
    override val values: Sequence<LocalModelEntity> = sequenceOf(
        ...
        LocalModelEntity(
            id = 5,
            modelId = "example/Wan2.1-T2V-GGUF",
            filename = "model.gguf",
            localPath = "/data/models/wan.gguf",
            sizeBytes = 4_000_000_000L,
            downloadedAt = Clock.System.now().epochSeconds,
            author = "example",
            libraryName = "gguf",
            pipelineTag = PipelineTag.TEXT_TO_VIDEO.apiValue,
            usageCount = 0,
            contextLength = null
        ),
        LocalModelEntity(
            id = 6,
            modelId = "example/wan-diffusers-bundle",
            filename = DIFFUSERS_BUNDLE_DB_FILENAME,
            localPath = "/data/models/wan_bundle",
