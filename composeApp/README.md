...
- Repair single-flight uses exact validated owner identity while normalized hashes only select bounded stripes; a cancelled leader exposes a typed abort and all live followers share one bounded successor, while each caller's own cancellation remains local
