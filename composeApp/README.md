...
- Download batches now persist strict versioned model evidence, bind the evidence digest into idempotent batch identity, and migrate v1 rows through nullable columns with exact-artifact enrichment recovery
- Language and smart diffusion enqueues now attach exact full-set descriptor evidence when available, while Needs information and descriptor mismatches remain downloadable with enrichment-only evidence
- Installed Ready models now repair missing descriptor evidence once from exact repository, revision, path, size, and canonical object identity, then reuse the strict record offline without legacy file inference
- Installed selections now resolve through manifest-only artifact evidence, fresh resource/settings snapshots, bounded shared workloads, and exact assessment-plan bindings before native loading
