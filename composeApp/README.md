...
- Durable download batches now retain strict versioned exact descriptor evidence for complete artifact sets, bind its digest into idempotent batch identity, and keep uncertain descriptors enrichment-only
...
- Repair single-flight uses exact validated owner identity while normalized hashes only select bounded stripes; queued manifest checks and owner-lock waits preserve cancellation, and Model Hub can publish Ready state only through its required durable coordinator
...
- Quarantined-load permission and exact retry now share one model-load job and an atomic ownership gate across runner release/load and terminal UI commits, so a newer selection cannot be overwritten by stale retry work
