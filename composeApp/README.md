...
- Model Hub now uses one compact registry hierarchy for Discover and Library, while Details prioritizes exact artifacts and keeps metadata and actions reachable at compact widths and large text
...
- Final verification passed 165 focused UI tests, 728 Compose JVM tests, the 852-test project JVM gate plus 4 native tests, Android debug assembly, and iOS simulator compilation; the connected Pixel remained securely locked, so the final device visual matrix is explicitly unverified
