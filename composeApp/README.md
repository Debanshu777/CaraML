...
- Existing Ready models repair missing descriptor metadata once from exact repository, revision, path, size, and canonical object identity, then reuse the persisted complete evidence offline without legacy file inference
- Native inference now accepts only a freshly assessed exact `LoadRequest` rebuilt from manifest artifacts, current resource/settings snapshots, bounded workloads, and an exact assessment-plan binding
