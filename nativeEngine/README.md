...
2. Extend CMake in this module: either replace `GgmlLlama.cmake` with a unified script that builds GGML once, then `add_subdirectory(llama.cpp)` and the new project (both must support `if (NOT TARGET ggml)`).
